...
def _add_common_arguments(parser):
    """
    Arguments shared by parse_inputs() and parse_synthetic_inputs().
    """
    parser.add_argument(
        "--prec",
    ...
    parser.add_argument(
        "--emin",
        type=float,
        help="Minimum energy at which the spectral density is evaluated. Default=1e-2.",
        default=1e-2,
    )
    parser.add_argument(
        "--e0",
    ...
    parser.add_argument(
        "--loglevel",
        type=str,
        help="Accepted strings are 'WARNING', 'INFO' or 'DEBUG'. Default='WARNING'. Setting 'INFO' leads to an extensive output tracking the details of the scan over lambda and alpha.",
        default="WARNING",
    )


def _assign_common_values(inputs, args):
...
def parse_inputs():
    ...
    _assign_common_values(inputs, args)
    return inputs
