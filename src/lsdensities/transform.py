...
def h_Et_mp_Eslice(Tinv_, params, estar_, alpha_):
    ...
    for i in range(params.tmax):
        for j in range(params.tmax):
            ht_[i] += Tinv_[i, j] * ft_mp(e=estar_, t=mpf(j + 1), sigma_=params.mpsigma, alpha=mpf(alpha_), e0=params.mpe0, type=params.periodicity, T=params.time_extent, ker_type=params.kerneltype)
    return ht_
