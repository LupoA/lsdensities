...
class obs:
    ...
    def evaluate(self, resampled=False):
        ...
        for i in range(self.T):
            self.central[i], self.sigma[i] = averageVector_fp(self.sample[:,i], get_error=True,  get_var=True)
        if resampled==False:
    ...
    def plot(self, show=True, logscale=True, label=None, yscale=1):
        ...
        if logscale == True:
            plt.yscale('log')
        plt.errorbar(x=list(range(0, self.T)), y=self.central/yscale, yerr=self.err/yscale, marker="o", markersize=1.5, elinewidth=1, ls='', label=label, color='#377eb8')
...
def init_precision(digits_):
    ...
    print(LogMessage(), "Setting precision ::::", "Approximate decimal precision: ", mp.dps)
