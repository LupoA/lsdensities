...
def h_Et_mp(Tinv_, params, espacemp_):                                 #     h(t,E) = sum_{r=0}    Tinv[t][r] * b[r+1,E]
    ht_ = mp.matrix(params.Ne,params.tmax)                             #     r+1 only in b, since T is already shifted
    for e in range(params.Ne):
        for i in range(params.tmax):
            ht_[e, i] = 0
            for j in range(params.tmax):
                aux_ =  mp.fmul(Tinv_[j,i],ft_mp(espacemp_[e], mpf(j+1), params.mpsigma, params.mpalpha, params.mpemin))
...
def h_Et_mp_Eslice(Tinv_, params, estar_):
    ...
    for i in range(params.tmax):
        ht_[i] = 0
        for j in range(params.tmax):
            aux_ =  mp.fmul(Tinv_[j,i], ft_mp(estar_, mpf(j+1), params.mpsigma, params.mpalpha, params.mpemin))
