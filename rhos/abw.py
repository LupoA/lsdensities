...
def gAg_periodic(smat, gt, estar, params):
    tmax = params.tmax
    term1 = mpf(0)
    vt_ = mp.matrix(tmax,1)
    for t in range(tmax):
    ...
    term3 = mpf(0)
    for t in range(tmax):
        aux_ = mp.fmul(ft_mp_periodic(tmax, e=estar, t=mpf(t+1), sigma_ = params.mpsigma, alpha=params.mpalpha, emin=params.mpemin), gt[t])
        term3 = mp.fadd(term3, aux_)
    term3 = mp.fmul(mpf(2), term3)
    #   term 3 = 2 sum_t f_t g_t
...
def gWg_periodic(smat,cov,gt,estar,mplambda, a0_,bnorm, params, verbose=False):
    aterm = gAg_periodic(smat,gt,estar,params)
    aterm = mp.fdiv(aterm, a0_)
    if verbose==True:
        print(LogMessage(), 'ABW :::', 'Energy {:2.3f}'.format(float(estar)), 'Lambda {:2.3f}'.format(float(mplambda)), 'A/A0 = ', float(aterm))
    bterm = gBg(gt,cov,bnorm)
...
def getLstar_Eslice_periodic(estar, Smat, a0_estar, CovDmat, csq, params, eNorm_=False, lambda_min=0.01, lambda_max=0.6, num_lambda=20):
    tmax = params.tmax
    lset = np.linspace(lambda_min, lambda_max, num_lambda)
    Wvec = np.zeros(num_lambda)
    lstar_ID = 0
    ...
    for li in range(num_lambda):
        ...
        invW = W**(-1)  # slow!!!
        #   given W, get the coefficient
        gtestar = h_Et_mp_Eslice_periodic(invW, params, estar, tmax)
        Wvec[li] = float(gWg_periodic(Smat, CovDmat, gtestar, estar, mp_l, a0_estar, Bnorm, params, verbose=True))
        if Wvec[li] > Wstar:
            Wstar = Wvec[li]
            lstar_ID = li
    print(LogMessage(), 'Lambda ::: ', "Lambda* at E = ", float(estar), ' ::: ', lset[lstar_ID])
