...
def gAg(smat, gt, estar, params):
    ...
    term3 = mpf(0)
    for t in range(tmax):
        aux_ = mp.fmul(ft_mp(tmax,e=estar, t=mpf(t+1), sigma_=params.mpsigma, alpha=params.mpalpha, emin=params.mpemin), gt[t])
...
def gAgA0(smat, gt, estar, params, a0):
    ...
    term3 = 0
    for t in range(tmax):
        aux_ = mp.fmul(ft_mp(tmax,estar, mpf(t+1), params.mpsigma, params.mpalpha, params.mpemin), gt[t])
        term3 = mp.fadd(term3, aux_)
    term3 = mp.fmul(mpf(2), term3)
